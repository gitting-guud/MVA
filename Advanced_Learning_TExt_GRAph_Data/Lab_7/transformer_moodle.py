...
class Transformer(Module):
    ...
    def forward(self, x, y=None):
        ...
        """
        ### YOUR CODE HERE ###
        input_Emb = self.source_embedding(x).double()
        input_Emb_PE = self.PE_tensor[:input_Emb.size(1),] + input_Emb
        if self.training and not y is None:
            ### YOUR CODE HERE ###
            y = self.shift(y)
            target_Emb = self.target_embedding(y).double()
            target_Emb_PE = self.PE_tensor[:target_Emb.size(1),]  + target_Emb
            
            enc_output = self.encoder(input_Emb_PE)
            enc_output, dec_output = self.decoder(enc_output, target_Emb_PE)
            out = self.output_linear(dec_output.float())
            out = out[:,1:,:]
        else:
            ### YOUR CODE HERE ###
            enc_output = self.encoder(input_Emb_PE)
            out = [self.SOS_token]
            for w in enc_output.squeeze() :
                Emb_out = self.target_embedding(out).double()
                target_Emb_out= self.target_embedding(Emb_out).double()
                target_Emb_PE_out = self.PE_tensor[:target_Emb_out.size(1),]  + target_Emb_out
                enc_output, dec_output = self.decoder(enc_output, target_Emb_PE_out)
                out = self.output_linear(dec_output.float())
...
class EncoderStack(Module):
    ...
    def forward(self, x):
        ...
        transformed_skip = interm + F.layer_norm(ff, normalized_shape=ff.size()[1:]).double()
...
class DecoderStack(Module):
    ...
    def forward(self, x, y):
        ...
        """
        ### YOUR CODE HERE ###
        interm = self.attention_1(y, y, y, maskout=True)
        interm_1 = y + F.layer_norm(interm, normalized_shape=interm.size()[1:])
        
        interm_2 = self.attention_2(Q=interm_1, K=x, V=x)
        interm_3 = interm_1 + F.layer_norm(interm_2,  normalized_shape=interm_2.size()[1:])
        
        ff_out = self.feed_forward(interm_3.float())
        transformed_skip = interm_3 + F.layer_norm(ff_out, normalized_shape=ff_out.size()[1:]).double()
...
class MultiHeadAttention(Module):
    ...
    def forward(self, Q, K, V, maskout=False):
        ...
        """
        ### YOUR CODE HERE ###
        Q_ = torch.unsqueeze(input=Q, dim=1) # dim (B, 1, T, dmodel)
        att_term_Q = torch.matmul(Q_, self.Wq.double()) # dim (B, Nheads, T, dk)
        att_term_K = torch.matmul(torch.unsqueeze(input=K, dim=1), self.Wk.double())# dim (B, Nheads, T, dk)
        att_term_V = torch.matmul(torch.unsqueeze(input=V, dim=1), self.Wv.double())# dim (B, Nheads, T, dv)
        dot_prod = torch.matmul(att_term_Q, att_term_K.permute(0,1,3,2))
        if maskout :
            mask_upper_tri = np.triu(np.full([att_term_K.shape[2], att_term_K.shape[2]], -np.inf), 1)
            torch_mask = torch.from_numpy(mask_upper_tri).type(torch.DoubleTensor)
            torch_mask = torch_mask.unsqueeze(0).unsqueeze(1).to(self.device)
            dot_prod = dot_prod + torch_mask
        
        softmax_term = F.softmax(dot_prod, dim=2)/ np.sqrt(self.dk)# dim (B, Nheads, T, T)
        attention = torch.matmul(softmax_term, att_term_V) # dim (B, Nheads, T, dv)
        attention = attention.permute(1,0,2,3) #(Nheads, B, T, dv)
        multihead = torch.cat(tuple(attention), dim=2)  # (B, T, dv*Nheads) = (B, T, dmodel)
        return multihead
...
if __name__ == "__main__":
    ...
    if os.path.exists("trained_transformer.pt"):
        ...
        model.fit(pairs_train, pairs_test, 20, warmup_step=4000)
