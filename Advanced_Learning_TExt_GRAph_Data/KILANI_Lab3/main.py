...
from keras.layers import Input, Embedding, Dropout, Conv1D, GlobalMaxPooling1D, Concatenate, Dense, Activation
...
#    return Dropout(d_rate)(GlobalMaxPooling1D()(Conv1D(filters=n_filters, kernel_size=k_size)(my_input)))
...
doc_wv = Embedding(input_dim=len(word_to_index)+2,
                   output_dim=d, 
                   input_length=max_size)(doc_ints)
...
for idx in range(nb_branches):
    ...
    branch_outputs.append(cnn_branch(n_filters = nb_filters,
                           k_size = filter_sizes[idx],
                           d_rate = drop_rate,
                           my_input = doc_wv_dr))
...
### fill the gap ###
### create a K.function '()' that returns the inner representation of the model before the prediction layer ###
...
### see: How can I obtain the output of get_doc_embedding an intermediate layer? here: https://keras.io/getting-started/faq/ ###
