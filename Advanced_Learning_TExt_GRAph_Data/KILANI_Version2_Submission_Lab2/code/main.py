...
def sample_examples(docs,max_window_size,n_windows):
    ...
    for i,doc in enumerate(docs):
        ...
        window_size = np.random.randint(n_windows)
        windows.append(list(get_windows(doc,window_size)))
    ...
    all_negs = list(np.random.choice(list(vocab_inv.keys()),
                                     size = n_negs*len(windows),
                                     p = neg_distr)) 
...
def compute_gradients(pos,negs,target,prodpos,prodnegs):
    ...
    partials_pos = np.tile(-Wt[target],(factors_pos.shape[0],1)) * factors_pos[:,np.newaxis]
    partials_negs = np.tile(Wt[target],(factors_negs.shape[0],1)) * factors_negs[:,np.newaxis]
    
    term_pos = - Wc[pos,] * factors_pos[:,np.newaxis]
    term_negs =  Wc[negs,] * factors_negs[:,np.newaxis]
...
#        partials_pos.extend([factors_pos[i] * (- Wt[target])])
#        term_pos.extend([-factors_pos[i] * Wc[ind]])
#    partials_pos = np.array(partials_pos)
#    term_pos = np.array(term_pos)
#    
#    partials_negs = []
#    term_negs = []  
#    for i,ind in enumerate(negs) :
#        partials_negs.extend([factors_negs[i] * Wt[target]])
#        term_negs.extend([factors_negs[i] * Wc[ind]])
#    partials_negs = np.array(partials_negs)
#    term_negs = np.array(term_negs)
...
path_read = "D:/MVA/Altegrad/Lab_2/for_moodle/for_moodle/data/"
...
if not train:
    ...
    cos_sim = my_cos_similarity("movie","banana")
    # = = visualization of most frequent tokens = =
    print("Similarity between 'movie' et 'banana' : ", cos_sim)
    n_plot = 250
    ...
    fig, ax = plt.subplots()
    ax.scatter(my_tsne_fit[:,0],
               my_tsne_fit[:,1],s=3) ### fill the gap ###
