...
path_to_abstracts = "D:/MVA/Altegrad/Lab_1/for_moodle/data/Hulth2003testing/abstracts"
...
path_to_keywords = "D:/MVA/Altegrad/Lab_1/for_moodle/data/Hulth2003testing/uncontr/"
...
for counter,g in enumerate(gs):
    ...
    itemMaxValue = max(core_numbers.items(), key=lambda x: x[1])
    listOfKeys = list()
    for key, value in core_numbers.items():
        if value == itemMaxValue[1]:
            listOfKeys.append(key)
    keywords['kc'].append(listOfKeys)
    ...
    for key, value in core_numbers.items():
        if value == itemMaxValue[1]:
            listOfKeys.append(key)
    keywords['wkc'].append(listOfKeys)
...
for counter,vector in enumerate(vectors_list):
    ...
    nonzero = filter(lambda x: x[1] != 0, terms_weights)
