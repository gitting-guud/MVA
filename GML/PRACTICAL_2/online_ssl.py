...
class IncrementalKCenters:
    ...
    def online_ssl_update_centroids(self, face):
        ...
        if self.centroids.shape[0] >= self.max_num_centroids + 1:
            ...
            euc_distances = distance.cdist( self.centroids, face.reshape(1,-1))
            sorted_index = np.argsort(euc_distances.reshape(-1))
            
            c_1 = sorted_index[0]
            c_1_in_taboo = self.taboo[c_1]
            if c_1_in_taboo :
                i = 1
                while i < len(sorted_index) and self.taboo[sorted_index[i]] :
                    i+=1
                i -= 1
                c_2, c_2_in_taboo = sorted_index[i], False
            else :
                c_2, c_2_in_taboo = sorted_index[1], self.taboo[sorted_index[1]]
...
#        Q = L + laplacian_regularization*np.eye(W.shape[0])
        ...
        """
        num_samples = np.size(self.centroids.shape, 0)
        Cl = np.unique(self.Y)
        ...
        for ind,c in enumerate(self.Y[l_idx]) :
            if c != 0 :
                f_l[ind][int(c)-1] = 1
        ...
        for b0 in box:
            ...
            if False:
                ...
                for i, l in enumerate(lab):
                    color = [(0, 255, 0), (255, 0, 0), (0, 0, 255)][1]
                    if np.abs(f[l]) > 0.6 :
